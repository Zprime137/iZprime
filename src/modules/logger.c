...
void log_test(int result, int test_id, const char *unit_name, const char *format, ...)
{
    va_list args;
    va_start(args, format);
    char message[1024];
...
static void print_line(int length, char fill_char)
{
...
void log_test_summary(char *module_name, int passed, int failed, int verbose)
{
    ...
    if (failed == 0)
    {
        printf("[SUCCESS] ALL %s TESTS PASSED!", module_name);
    }
    else
    {
        printf("[FAILURE] SOME %s TESTS FAILED :\\", module_name);
        if (!verbose)
